...
public class SoapValidationService(ICertificateProviderService certificateProviderService): ISoapValidationService
{
  ...
  private readonly string _encNs = "http://www.w3.org/2001/04/xmlenc#";
  ...
  public void Validate(XDocument soapDocument)
  {
    ...
    _ = body?.Element(encNs + "EncryptedData") ?? throw new SoapFaultException("env:Sender", @"Message could not be parsed, missing ""EncryptedData"" element");
    ...
    _ = security?.Element(encNs + "EncryptedKey") ?? throw new SoapFaultException("env:Sender", @"Message could not be parsed, missing ""EncryptedKey"" element");
  }
