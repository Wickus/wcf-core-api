...
public class CertificateProviderService(IConfiguration configuration): ICertificateProviderService
{
  ...
  {
    string pfxPath = _configuration["CertificateSettings:Path"] ?? throw new Exception("Certificate Path not set");
    string pfxPassword = _configuration["CertificateSettings:Password"] ?? throw new Exception("Certificate Password not set");

    if (string.IsNullOrEmpty(pfxPath))
      throw new ArgumentException("Certificate Path is not provided.");
  ...
  public RSA? GetPrivateKey() => GetServiceCertificate()?.GetRSAPrivateKey();

  public RSA? GetPublicKey() => GetServiceCertificate()?.GetRSAPublicKey();
}
